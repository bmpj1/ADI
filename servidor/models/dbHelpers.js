 ...
 * @author Brian Mathias Pesci Juliani
 * @param {string} tabla - String con el nombre de la tabla a trabajar
 * @param {number} perPage - Numero con la cantidad de items por pagina
 * @param {number} currentPage - Numero con la pagina actual (la que se quiere obtener)
 * @returns {Array} - Array con todas los datos de la tabla
 */
async function tablaPaginada(tabla, perPage, currentPage) {
    const result = await db(tabla).paginate({perPage: perPage, currentPage: currentPage})
    return result
}
